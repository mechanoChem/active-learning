import os,sys
import numpy as np

directory = os.path.abspath('../../../active-learning/')
sys.path.insert(0,directory)
from active_learning.workflow.dictionary import Dictionary

input_path = 'input_predicting.ini'
dictionary = Dictionary(input_path)
from active_learning.model.idnn_model import IDNN_Model 
model = IDNN_Model(dictionary)



for rnd in [5]:

    model.load_trained_model(rnd)

    data = '2d_slice_rnd21_0_1.txt'
    eta = np.genfromtxt(data,dtype=np.float32)[:,:4]
    T = np.ones((np.shape(eta)[0],1))*260
    # file1 = '../../Output/Output8/CASMallResults15.txt'
    # eta = np.genfromtxt(file1,dtype=np.float32)[:,4:8]
    # mu = np.genfromtxt(file1,dtype=np.float32)[:,13:17]*100
    # T= np.genfromtxt(file1,dtype=np.float32)[:,12:13]
    eta_test = eta.copy()
    # eta_test[:,0] = (eta[:,0] - .5)*2
    # eta_test[:,1] = (eta[:,1])*2

    pred_new = model.predict([eta_test,T])

    free_new = pred_new[0]/100
    mu_new = pred_new[1]
    print(mu_new)
    # mu = pred[1]
    grad = pred_new[2]
    np.savetxt('../../Output/prediction_output8_rnd{}.txt'.format(rnd),np.hstack((eta,mu_new,free_new)))