...
def write(OutputFolder,rnd):
                        ...
                        with open(OutputFolder + 'data/data_sampled/round_{}/'.format(rnd)+dir+'/results.json','r') as file:
                            ...
                            for i in range(len(data['<comp(a)>'])):
                                try:
                                    ...
                                    print('copy incomplete ',dir)
                except:
                    print('larger error ',dir)
        ...
        np.savetxt(OutputFolder + 'data/data_sampled/CASMresults{}.txt'.format(rnd),
                dataOut,
                header=header)
        if rnd==0:
            copyfile(OutputFolder + 'data/data_sampled/CASMresults{}.txt'.format(rnd),OutputFolder + 'data/data_sampled/CASMallResults{}.txt'.format(rnd))
        else:
            allResults = np.loadtxt(OutputFolder + 'data/data_sampled/CASMallResults{}.txt'.format(rnd-1))
            allResults = np.vstack((allResults,dataOut))
            np.savetxt(OutputFolder + 'data/data_sampled/CASMallResults{}.txt'.format(rnd),
                    allResults,
                    header=header)
        ...
        np.save(OutputFolder + 'data/data_sampled/results{}'.format(rnd),
        dataOut)
        if rnd==0:
            np.save(OutputFolder + 'data/data_sampled/allResults{}'.format(rnd),
                    dataOut)
        else:
            allResults =  np.load(OutputFolder + 'data/data_sampled/results{}.npy'.format(rnd-1),allow_pickle=True)
            allResults = np.vstack((allResults,dataOut))

            np.save(OutputFolder + 'data/data_sampled/allResults{}'.format(rnd),
                    allResults)
        
        np.savetxt(OutputFolder + 'data/data_sampled/results{}.txt'.format(rnd),dataOut)
        


write('../../Output_2d/280_8/',int(rnd))