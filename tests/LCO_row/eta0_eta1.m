...
i=13;
for i=5
    var = ['../../Output/prediction_output8_rnd' num2str(i)   '.txt'];
    % var = ['../../Output/Output8/hp_prediction_' num2str(i)   '.txt'];
    % var = ['../../Output/260_5/graphs/rnd' num2str(i)   '.txt'];
    data= load(var);
    ...
    g= (reshape(data(:,end),[50,50])-min(data(:,end)))*1000;
    surf(reshape(data(:,1),[50,50]),reshape(data(:,2),[50,50]),g,min(g,.006))
    % eta0=data(:,1);
    % eta1=data(:,2);
    ...
    figure(2*i+2);clf
    scatter(data(:,1),data(:,5)*100)
    % xlim([0.45,0.55])

    var2 = ['../../Output/Output8/results15.txt'];
    data2 = load(var2);
    
    figure(2*i+3);clf
    scatter(data2(:,1),data2(:,5))
