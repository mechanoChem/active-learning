...
class Dictionary():
    ...
    def maintain_input(self):
            ...
            if self.dict[input]['domain_type']=='continuous_dependent':
                ...
                if domain not in self.dict['Sampling']['continuous_dependent']:
                    ...
                    if space_filling_method == 'billiardwalk':
                        ...
                        self.set_as_float([[domain,'2d_perturb'],[domain,'2d_points']])
                        self.set_as_true_false([[domain,'2d']])
                        self.dict['Sampling']['continuous_dependent'][domain]['x0'] = self.dict[domain]['x0']
                        self.dict['Sampling']['continuous_dependent'][domain]['2d'] = self.dict[domain]['2d']
                        self.dict['Sampling']['continuous_dependent'][domain]['2d_points'] = self.dict[domain]['2d_points']
                        self.dict['Sampling']['continuous_dependent'][domain]['2d_perturb'] = self.dict[domain]['2d_perturb']
        ...
        for domain in self.dict['Sampling']['continuous_dependent']:
            ...
            relevent_indices_2d = relevent_indices[0:2]
            invQ = np.linalg.inv(Q)[:,relevent_indices_2d]
            self.dict['Sampling']['continuous_dependent'][domain]['Q_2d'] = Q[:,relevent_indices_2d]
            self.dict['Sampling']['continuous_dependent'][domain]['n_planes_2d'] = np.vstack((invQ,-invQ))
            self.dict['Sampling']['continuous_dependent'][domain]['c_planes_2d']= np.hstack((np.ones(invQ.shape[0]),np.zeros(invQ.shape[0])))
            self.dict['Sampling']['continuous_dependent'][domain]['invQ_2d']= invQ
